        ...
        protected override void EndProcessing()
        {
            try
            {
                ...
                else if (Incomplete.IsPresent)
                {
                    ...
                    response = Task.Run(async () => await torrentSvc.RemoveTorrents(new List<int>{TorrentId.Value}, DeleteData.IsPresent)).Result;
                else
                    response = Task.Run(async () => await torrentSvc.RemoveTorrents(TorrentIds, DeleteData.IsPresent)).Result;
