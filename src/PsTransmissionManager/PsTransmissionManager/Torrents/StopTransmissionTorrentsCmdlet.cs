        ...
        protected override void EndProcessing()
        {
            try
            {
                ...
                else if (Incomplete.IsPresent)
                {
                    ...
                    response = Task.Run(async () => await torrentSvc.StopTorrents(new List<int> { TorrentId.Value })).Result;
                else
                    response = Task.Run(async () => await torrentSvc.StopTorrents(TorrentIds)).Result;
